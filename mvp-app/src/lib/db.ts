...
function migrate() {
  ...
  ensureColumn('lessons', 'duration_min', 'duration_min INTEGER DEFAULT 0')
  ensureColumn('lessons', 'content_md', 'content_md TEXT')
  ensureColumn('lessons', 'content_json', 'content_json TEXT')
...
// Ensure schema exists before checking meta flags
try { migrate() } catch {}
...
if (!metaSeeded?.value) {
  ...
  try { upgradeIntroFrontendIfNeeded() } catch {}
  try { upgradeIntroAnalyticsIfNeeded() } catch {}
  try { upgradeIntroUxIfNeeded() } catch {}
  try { deepSeedAnalyticsIfNeeded() } catch {}
  try { deepSeedUxIfNeeded() } catch {}
  // 3) enrich to add more lessons if needed
  try { enrichCoursesIfNeeded() } catch {}
